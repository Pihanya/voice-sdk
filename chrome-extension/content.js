...
a.src = chrome.runtime.getURL('annyang.js');
c.src = chrome.runtime.getURL('commands.js');
a.onload = function() {
    this.remove();
};
c.onload = function() {
...
for(var i = 0; i < inputs.length; i++) {
	...
	'login': function() {
		window.location.href='https://passport.yandex.ru/auth?origin=market_desktop_header&retpath=https://market.yandex.ru/';
	},
	'sale': function() {
		window.location.href='https://market.yandex.ru/catalog--tovary-so-skidkoi/61522';
	},
	'putin': function() {
		window.location.href='https://ru.wikipedia.org/wiki/%D0%9F%D1%83%D1%82%D0%B8%D0%BD,_%D0%92%D0%BB%D0%B0%D0%B4%D0%B8%D0%BC%D0%B8%D1%80_%D0%92%D0%BB%D0%B0%D0%B4%D0%B8%D0%BC%D0%B8%D1%80%D0%BE%D0%B2%D0%B8%D1%87';
	},
	...
	'filter by *filter': function(filter) {
		
var aTags = document.getElementsByTagName("a");
var searchText;
var found;
switch (filter) {
	case 'price': searchText = 'по цене'; break;
	case 'rating': searchText = 'по рейтингу'; break;
	case 'reviews': searchText = 'по отзывам'; break;
	case 'popularity': searchText = 'по популярности'; break;
	case 'sale': searchText = 'по размеру скидки'; break;
	case 'newest': searchText = 'по новизне'; break;
	default: return;
}
...
found.click();
	...
	'price from *from to *to': function(from, to) {
		const glpricefrom = document.getElementById('glpricefrom');
		const glpriceto = document.getElementById('glpriceto');
		glpricefrom.click();
		glpricefrom.value = from;
		glpriceto.click();
		glpriceto.value = to;
	}
  };
